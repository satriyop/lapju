...
class ProgressDataSeeder extends Seeder
{
    /**
     * Seed progress data for PT Adhi Karya project for the last 3 months.
     * Generates realistic S-curve data pattern.
     */
    public function run(): void
    {
        // Find PT Adhi Karya project
        $project = Project::whereHas('customer', fn ($q) => $q->where('name', 'like', '%ADHI KARYA%'))->first();

        if (! $project) {
            $this->command->error('PT Adhi Karya project not found!');
        ...
        // Define date range: last 3 months up to today
        ...
        $this->command->info("Successfully seeded {$totalEntries} progress entries for PT Adhi Karya project.");
